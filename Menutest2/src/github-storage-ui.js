...
document.addEventListener('DOMContentLoaded', function() {
                ...
                <div id="github-setup-form">
                    ...
                    <div class="form-group">
                        <label class="checkbox-label">
                            <input type="checkbox" id="github-autoload" checked>
                            <span>Tự động tải dữ liệu mới nhất khi khởi động</span>
                        </label>
                    </div>
    ...
    loadingIndicator.id = 'github-loading-indicator';
    loadingIndicator.textContent = 'Đang tải dữ liệu từ GitHub...';
    ...
    const autoloadCheckbox = document.getElementById('github-autoload');
    ...
    function showGitHubSetupModal() {
        ...
        if (window.gitHubStorage.loadConfig()) {
            ...
            const autoload = localStorage.getItem('github_autoload');
            autoloadCheckbox.checked = autoload === null || autoload === 'true';
    ...
    function saveGitHubConfig() {
        ...
        const autoload = autoloadCheckbox.checked;
        ...
        localStorage.setItem('github_autoload', autoload);
    ...
    async function syncAllDataToGitHub() {
        try {
            ...
            localStorage.setItem('github_last_sync', now.getTime().toString());
        } catch (error) {
    ...
    async function loadAllDataFromGitHub(silent = false) {
        try {
            if (!silent) {
            ...
            localStorage.setItem('github_last_load', new Date().getTime().toString());
            ...
            if (!silent) {
                ...
                // Reload without confirmation in silent mode
                window.location.reload();
            }
    ...
    // Function to auto-load data at startup
    async function autoLoadData() {
        // Check if autoload is enabled
        const autoload = localStorage.getItem('github_autoload');
        if (autoload === 'false') {
            console.log('Auto-load is disabled, skipping');
            return;
        }
        
        // Check if GitHub is configured
        if (!window.gitHubStorage.loadConfig()) {
            console.log('GitHub not configured, skipping auto-load');
            return;
        }
        
        // Get last load timestamp
        const lastLoad = localStorage.getItem('github_last_load');
        const now = new Date().getTime();
        
        // If we've loaded within the last hour, don't auto-load again
        if (lastLoad && (now - parseInt(lastLoad)) < 3600000) {
            console.log('Data loaded recently, skipping auto-load');
            return;
        }
        
        // Load data silently
        console.log('Auto-loading latest data from GitHub...');
        await loadAllDataFromGitHub(true);
    }
    ...
    // Run auto-load after a short delay to let the page load first
