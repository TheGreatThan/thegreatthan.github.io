document.addEventListener('DOMContentLoaded', function() {
    ...
    function saveToFile(data, filename) {
        // Always use client-side approach since GitHub Pages doesn't support PHP
        ...
        try {
            ...
            console.log(`Data also saved to localStorage with key: ${storageKey}`);
        } catch (error) {
    ...
    function exportAllStorageData() {
        ...
        for (let i = 0; i < localStorage.length; i++) {
            ...
            try {
                ...
                exportData.storageItems[key] = value;
            } catch (error) {
        ...
        const now = new Date();
        const dateStr = `${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, '0')}-${String(now.getDate()).padStart(2, '0')}`;
        a.download = `web_storage_backup_${dateStr}.json`;
        a.href = URL.createObjectURL(blob);
