document.addEventListener('DOMContentLoaded', function() {
    ...
    function saveToFile(data, filename) {
        // Phát hiện nếu đang chạy trên máy chủ web
        ...
        if (isServer) {
            ...
            .then(result => {
                if (result.success) {
                    console.log('Data saved successfully to server:', result.filepath);
                } else {
                    console.error('Failed to save data to server:', result.error);
                    // Fallback to client-side download
                    saveToFileClient(data, filename);
                }
            })
            .catch(error => {
            ...
            // Lưu file phía client nếu không chạy trên máy chủ
            saveToFileClient(data, filename);
        }
    }
    
    // Function to save data to a file on client-side (fallback)
    function saveToFileClient(data, filename) {
    ...
    function loadFileList(filter = null) {
        // Phát hiện nếu đang chạy trên máy chủ web
        const isServer = window.location.protocol !== 'file:';
        
        if (!isServer) {
            console.log('Cannot load file list when not running on a server');
            return Promise.resolve([]);
        }
        
        let url = 'listFiles.php';
        if (filter) {
    ...
    // Load data from a server file
    function loadFileFromServer(filename) {
        return fetch(`loadData.php?filename=${encodeURIComponent(filename)}`)
            .then(response => response.json())
            .then(result => {
                if (result.success) {
    ...
    function loadStorageFiles(filter = null) {
        ...
        loadFileList(filter)
                    ...
                    const row = `
                        <tr>
                            ...
                            <td class="file-actions">
                                <button class="btn-load-file" data-file="${file.name}">Tải</button>
                                <button class="btn-delete-file" data-file="${file.name}">Xóa</button>
                            </td>
                        </tr>
                    `;
                    
                    storageFilesList.innerHTML += row;
                });
                    ...
                    button.addEventListener('click', function() {
                        ...
                        loadFileFromServer(filename)
                            .then(data => {
                                ...
                                } else if (filename.startsWith('all_data_')) {
                                    // Load all data
                                    if (data.orders) {
                                        orders = data.orders;
                                        saveOrders();
            ...
            .catch(error => {
                ...
                storageFilesList.innerHTML = '<tr><td colspan="4" class="loading-files">Lỗi khi tải danh sách file</td></tr>';
            });
    }

    // Delete file from server
    function deleteFileFromServer(filename) {
        return fetch('deleteFile.php', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
