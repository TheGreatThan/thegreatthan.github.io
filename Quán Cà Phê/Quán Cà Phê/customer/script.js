...
function sendOrderToOwner(order) {
    // Store the order in localStorage for simplicity
    // In a real app, this would send data to a server
    let orders = JSON.parse(localStorage.getItem('coffee_shop_orders') || '[]');
    orders.push(order);
    localStorage.setItem('coffee_shop_orders', JSON.stringify(orders));
    
    // Notify the owner page if it's open (for demo purposes)
    if (window.opener) {
        window.opener.postMessage({ type: 'new_order', order: order }, '*');
    }
} 