...
import { getFirestore, collection, doc, setDoc, getDoc, addDoc, updateDoc, onSnapshot, orderBy, query, serverTimestamp, getDocs } from "firebase/firestore";
  ...
  loadMenu: async function() {
    try {
      ...
      if (menuDoc.exists()) {
        ...
        // Create default menu if no menu exists in the database
        const defaultMenu = [
        ...
        // Save the default menu to Firestore
        await this.saveMenu(defaultMenu);
        console.log("Created default menu in Firestore");
  ...
  listenForMenuChanges: function(callback) {
    return onSnapshot(doc(db, "settings", "menu"), (doc) => {
      if (doc.exists()) {
        ...
        console.warn("Menu document doesn't exist in Firestore");
      }
    }, (error) => {
      console.error("Error listening for menu changes:", error);
    });
  },
