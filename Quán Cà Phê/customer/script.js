...
async function initializeMenu() {
    try {
        ...
        console.log("Menu loaded from Firebase:", loadedMenu);
        ...
        if (loadedMenu) {
            ...
            console.warn("No menu returned from Firebase, using default");
            // Use default menu if Firebase returns null
            menuItems = [
        ...
        menuUnsubscribe = cloudStorage.listenForMenuChanges((updatedMenu) => {
            console.log("Menu updated from Firebase:", updatedMenu);
            if (updatedMenu) {
...
addToCartButtons.forEach(button => {
    button.addEventListener('click', () => {
        ...
        console.log("Add to cart clicked, item id:", id);
        console.log("Current menu items:", menuItems);
        
        // Check if item is available before adding to cart
        const item = menuItems.find(item => parseInt(item.id) === parseInt(id));
        console.log("Found menu item:", item);
        
        if (item && item.available !== false) {  // Check that available is not explicitly false
...
function updateMenuUI() {
    console.log("Updating menu UI with items:", menuItems);
    // Update each menu item based on availability
    const menuItemElements = document.querySelectorAll('.menu-item');
    
    menuItemElements.forEach(element => {
        const id = element.dataset.id;
        console.log(`Checking menu item with id ${id}`);
        
        // Find the item in our menuItems array, converting IDs to integers for comparison
        const menuItem = menuItems.find(item => parseInt(item.id) === parseInt(id));
        console.log(`Menu item found:`, menuItem);
        
        if (menuItem && menuItem.available === false) {  // Check that available is explicitly false
            ...
            // Item is available or no menu data, ensure it's enabled
